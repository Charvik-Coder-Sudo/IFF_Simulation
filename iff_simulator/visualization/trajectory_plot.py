"""Static matplotlib visualizations of the merged ground-truth dataset.
...
Engineering explanation:
    ...
    unbounded memory growth when many targets are plotted.
"""
...
class TrajectoryPlotter:
    """Renders diagnostic plots of the merged ground-truth dataset.

    Purpose:
        Provide a single reusable component that turns the merged
        ground-truth table into the standard set of diagnostic plots
        used to visually sanity-check recorded trajectories.
    ...
    def plot_trajectory_xy(self, output_dir: Path | str) -> Path:
        ...
        for target_id in self.inspector.list_targets():
            ...
            ax.plot(df["X"], df["Y"], label=target_id)
        ax.legend()
    ...
    def plot_trajectory_xz(self, output_dir: Path | str) -> Path:
        ...
        for target_id in self.inspector.list_targets():
            df = self.inspector.get_target(target_id)
            ax.plot(df["X"], df["Z"], label=target_id)
        ax.legend()
    ...
    def plot_speed_vs_time(self, output_dir: Path | str) -> Path:
        ...
        for target_id in self.inspector.list_targets():
            ...
            ax.plot(df["Time"], speed, label=target_id)
        ax.legend()
    ...
    def plot_altitude_vs_time(self, output_dir: Path | str) -> Path:
        ...
        for target_id in self.inspector.list_targets():
            df = self.inspector.get_target(target_id)
            ax.plot(df["Time"], df["Z"], label=target_id)
        ax.legend()
    ...
    def plot_final_positions(self, output_dir: Path | str) -> Path:
        ...
        for target_id in self.inspector.list_targets():
            df = self.inspector.get_target(target_id)
            final_row = df.iloc[-1]
            ax.scatter(final_row["X"], final_row["Y"], label=target_id, s=60)
        ax.legend()
